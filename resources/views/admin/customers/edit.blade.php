...
<div class="container">
    <div class="alert alert-success mt-2" id="success_msg" style="display:none">
      ...
      <form id="update_customer" enctype="multipart/form-data" method="POST" action="{{route('admin.customer.store')}}">
        @csrf
        @method('PUT')
...
<script>
    $(document).on('click', '#update', function (e) {
        e.preventDefault();
        var formData = new FormData($('#update_customer')[0]);
        $.ajax({
            type: 'post',
             enctype: 'multipart/form-data',
            url: "{{route('admin.customer.update',$user->id)}}",
            data: formData,
            processData: false,
            contentType: false,
            cache: false,
